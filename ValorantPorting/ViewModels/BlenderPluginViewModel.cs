...
public partial class BlenderPluginViewModel : ViewModelBase
{
    ...
    public async Task Sync(BlenderInstallInfo installInfo, bool automatic = false)
    {
        ...
        if (CheckBlenderRunning(installInfo.BlenderPath, automatic))
        {
            ...
            {
                MessageWindow.Show("An Error Occurred", $"ValorantPorting tried to auto sync the plugin, but an instance of blender is open. Please close it and sync the plugin in the plugin tab.");
            }
            return;
        }
        ...
        var assets = Avalonia.Platform.AssetLoader.GetAssets(new Uri("avares://ValorantPorting/Plugins/Blender"), null);
        ...
        try
        {
            ...
            blenderProcess.Exited += (sender, args) =>
            {
                ...
                {
                    MessageWindow.Show("An Error Occured", "Blender failed to enable the ValorantPorting plugin. If this is your first time using syncing the plugin, please enable it yourself in the add-ons tab in Blender preferences, otherwise, you may ignore this message.");
    ...
    {
        var initStream = Avalonia.Platform.AssetLoader.Open(new Uri("avares://ValorantPorting/Plugins/Blender/ValorantPorting/__init__.py"));
...
public partial class BlenderInstallInfo : ObservableObject
{
    ...
    {
        var initFilepath = Path.Combine(AddonBasePath, "ValorantPorting", "__init__.py");
