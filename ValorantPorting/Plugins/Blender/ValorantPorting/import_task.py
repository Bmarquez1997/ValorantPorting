...
default_mappings = MappingCollection(
    textures=[
        SlotMapping("Albedo"),
        # SlotMapping("D", "Albedo"),
        SlotMapping("DF", "Albedo", alpha_slot="Alpha"),
        SlotMapping("Diffuse", "Albedo"),
        SlotMapping("MaterialTexture", "Albedo"),
        SlotMapping("Diffuse B", "Albedo"),
        SlotMapping("Diffuse A", "Albedo"),
        SlotMapping("Base Color", "Albedo"),
        SlotMapping("Concrete", "Albedo"),
        SlotMapping("Trunk_BaseColor", "Albedo"),

        SlotMapping("MRAE"),
        SlotMapping("MRA", "MRAE"),
        SlotMapping("MRA B", "MRAE"),
        SlotMapping("MRA A", "MRAE"),

        SlotMapping("Normal"),
        SlotMapping("N", "Normal"),
        SlotMapping("Normal", "Normal"),
        SlotMapping("AdditionalNormal", "Normal"),
        SlotMapping("Normal 1", "Normal"),
        SlotMapping("Texture B Normal", "Normal"),
        SlotMapping("Texture A Normal", "Normal"),
        SlotMapping("NormalMap", "Normal"),
        SlotMapping("ConcreteTextureNormal", "Normal"),
        SlotMapping("Trunk_Normal", "Normal"),

        SlotMapping("Decal Color Texture"),
        SlotMapping("Decal Mask Texture", alpha_slot="Decal Mask Texture"),
        SlotMapping("Decal MRAE Texture"),
        SlotMapping("Decal Normal Texture")
    ],
    scalars=[
        SlotMapping("Skin Intensity"),
...
eye_mappings = MappingCollection(
    textures=[
        SlotMapping("OverlayTexture", alpha_slot="OverlayTextureAlpha"),
    ]
)
...
class ImportTask:
    def run(self, response):
        ...
        append_data(options.get("UseOctaneShaders"))
        # append_data()
...
class DataImportTask:
    ...
    def import_data(self, data):
        ...
        self.useOctane = self.options.get("UseOctaneShaders")
    ...
    def import_mesh_data(self, data):
        ...
        self.collection = create_collection(self.name) if self.options.get(
            "ImportCollection") else bpy.context.scene.collection
        ...
        if self.type == "Agent":
            for imported_mesh in self.imported_meshes:
                master_skeleton = imported_mesh.get("Skeleton")
                ...
                if self.rig_type == ERigType.CUSTOM:
                    apply_valorant_rig(master_skeleton)
    ...
    def import_anim_data(self, data, override_skeleton=None):
        ...
        if target_skeleton is None:
            MessageServer.instance.send(
                "An armature must be selected to import an animation. Please select an armature and try again.")
            return

        if target_skeleton.get("is_tasty_rig"):
            MessageServer.instance.send(
                "Tasty Rig currently does not support emotes. Please use a character with the Default Rig and try again.")
            return
    ...
    def import_material(self, material_slot, material_data, meta_data):
        ...
        if (name_existing := first(self.imported_materials.items(),
                                   lambda x: x[1].name.casefold() == material_name.casefold())) and name_existing[
            0] != material_hash:
            material_name += f"_{hash_code(material_hash)}"
        ...
        shader_node.node_tree = bpy.data.node_groups.get("3P_Character_Mat_V5")
        ...
        layer_switch_names = ["Use 2 Layers", "Use 3 Layers", "Use 4 Layers", "Use 5 Layers", "Use 6 Layers",
                              "Use 7 Layers",
                              "Use 2 Materials", "Use 3 Materials", "Use 4 Materials", "Use 5 Materials",
                              "Use 6 Materials", "Use 7 Materials",
        ...
        if "Eye_MI" in material_name:
            replace_shader_node("3P_EyeOverlay_Mat")
            socket_mappings = eye_mappings

        # if self.type == "World":
        #     replace_shader_node("FP Material")
        #     socket_mappings = eye_mappings
        # 
        # if self.type == "Weapon":
        #     replace_shader_node("3P_EyeOverlay_Mat")
        ...
        if shader_node.node_tree.name == "FP Material":
            ...
            if (skin_color := meta_data.get("SkinColor")) and skin_color["A"] != 0:
                shader_node.inputs["Skin Undercolor"].default_value = (
            ...
            if (crop_bounds := get_param_multiple(vectors, emission_crop_vector_params)) and get_param_multiple(
                    switches, emission_crop_switch_params) and len(emission_slot.links) > 0:
            ...
            if eye_texture_data := get_param_data(textures, "EyeTexture"):
                ...
                eye_texture_node.image.colorspace_settings.name = "sRGB" if eye_texture_data.get(
                    "sRGB") else "Non-Color"
    ...
    def import_mesh(self, path: str, num_lods):
        ...
        lod_mesh_path = os.path.join(self.assets_folder, path.split(".")[
            0] + f"_LOD{str(min(num_lods - 1, self.options.get('LevelOfDetail')))}" + ".uemodel")
...
def append_data(isOctaneShader):
    append_agent_data(isOctaneShader)
    append_weapon_data(isOctaneShader)

    addon_dir = os.path.dirname(os.path.splitext(__file__)[0])
    with bpy.data.libraries.load(os.path.join(addon_dir, "valorant_porting_data.blend")) as (data_from, data_to):
...
def append_agent_data(isOctane):
    addon_dir = os.path.dirname(os.path.splitext(__file__)[0])
    data_file_name = "Octane/VALORANT_Agent.blend" if isOctane else "VALORANT_Agent.blend"
    with bpy.data.libraries.load(os.path.join(addon_dir, data_file_name)) as (data_from, data_to):
        for node_group in data_from.node_groups:
            if not bpy.data.node_groups.get(node_group):
                data_to.node_groups.append(node_group)


def append_weapon_data(isOctane):
    addon_dir = os.path.dirname(os.path.splitext(__file__)[0])
    data_file_name = "Octane/VALORANT_Weapon.blend" if isOctane else "VALORANT_Weapon.blend"
    with bpy.data.libraries.load(os.path.join(addon_dir, data_file_name)) as (data_from, data_to):
        for node_group in data_from.node_groups:
            if not bpy.data.node_groups.get(node_group):
                data_to.node_groups.append(node_group)
...
def merge_skeletons(parts):
    ...
    for part in parts:
        if (meta := part.get("Meta")) and meta.get("AttachToSocket") and meta.get("Socket") not in ["Face", "Helmet",
                                                                                                    None]:
...
def apply_tasty_rig(master_skeleton, scale, use_finger_ik = True, use_dyn_bone_shape = True):
    ...
    new_bones = [
        ("tasty_root", "root",
         LazyInit(lambda: (edit_bones["root"].head, edit_bones["root"].tail, edit_bones["root"].roll))),

        ("ik_foot_parent_r", "tasty_root",
         LazyInit(lambda: (edit_bones["foot_r"].head, edit_bones["foot_r"].tail, edit_bones["foot_r"].roll))),
        ("ik_foot_ctrl_r", "ik_foot_parent_r", LazyInit(lambda: (
            edit_bones["ball_r"].head + Vector((0, 0.2, 0)) * scale,
            edit_bones["ball_r"].tail + Vector((0, 0.2, 0)) * scale, 0))),
        ("ik_foot_roll_inner_r", "ik_foot_parent_r", LazyInit(lambda: (
            Vector((edit_bones["ball_r"].head.x + 0.04 * scale, edit_bones["ball_r"].head.y, 0)),
            Vector((edit_bones["ball_r"].tail.x + 0.04 * scale, edit_bones["ball_r"].tail.y, 0)), 0))),
        ("ik_foot_roll_outer_r", "ik_foot_roll_inner_r", LazyInit(lambda: (
            Vector((edit_bones["ball_r"].head.x - 0.04 * scale, edit_bones["ball_r"].head.y, 0)),
            Vector((edit_bones["ball_r"].tail.x - 0.04 * scale, edit_bones["ball_r"].tail.y, 0)), 0))),
        ("ik_foot_roll_front_r", "ik_foot_roll_outer_r",
         LazyInit(lambda: (edit_bones["ball_r"].head, edit_bones["ball_r"].tail, radians(180)))),
        ("ik_foot_roll_back_r", "ik_foot_roll_front_r", LazyInit(lambda: (
            Vector((edit_bones["foot_r"].head.x, edit_bones["foot_r"].head.y + 0.065 * scale, 0)),
            Vector((edit_bones["foot_r"].tail.x, edit_bones["foot_r"].tail.y + 0.065 * scale, 0)), 0))),
        ("ik_foot_target_r", "ik_foot_roll_back_r",
         LazyInit(lambda: (edit_bones["foot_r"].head, edit_bones["foot_r"].tail, edit_bones["foot_r"].roll))),
        ("ik_foot_pole_r", "tasty_root", LazyInit(lambda: (edit_bones["calf_r"].head + Vector((0, -0.75, 0)) * scale,
                                                           edit_bones["calf_r"].head + Vector((0, -0.75, -0.2)) * scale,
                                                           0))),

        ("ik_foot_parent_l", "tasty_root",
         LazyInit(lambda: (edit_bones["foot_l"].head, edit_bones["foot_l"].tail, edit_bones["foot_l"].roll))),
        ("ik_foot_ctrl_l", "ik_foot_parent_l", LazyInit(lambda: (
            edit_bones["ball_l"].head + Vector((0, 0.2, 0)) * scale,
            edit_bones["ball_l"].tail + Vector((0, 0.2, 0)) * scale, 0))),
        ("ik_foot_roll_inner_l", "ik_foot_parent_l", LazyInit(lambda: (
            Vector((edit_bones["ball_l"].head.x - 0.04 * scale, edit_bones["ball_l"].head.y, 0)),
            Vector((edit_bones["ball_l"].tail.x - 0.04 * scale, edit_bones["ball_l"].tail.y, 0)), 0))),
        ("ik_foot_roll_outer_l", "ik_foot_roll_inner_l", LazyInit(lambda: (
            Vector((edit_bones["ball_l"].head.x + 0.04 * scale, edit_bones["ball_l"].head.y, 0)),
            Vector((edit_bones["ball_l"].tail.x + 0.04 * scale, edit_bones["ball_l"].tail.y, 0)), 0))),
        ("ik_foot_roll_front_l", "ik_foot_roll_outer_l",
         LazyInit(lambda: (edit_bones["ball_l"].head, edit_bones["ball_l"].tail, radians(180)))),
        ("ik_foot_roll_back_l", "ik_foot_roll_front_l", LazyInit(lambda: (
            Vector((edit_bones["foot_l"].head.x, edit_bones["foot_l"].head.y + 0.065 * scale, 0)),
            Vector((edit_bones["foot_l"].tail.x, edit_bones["foot_l"].tail.y + 0.065 * scale, 0)), 0))),
        ("ik_foot_target_l", "ik_foot_roll_back_l",
         LazyInit(lambda: (edit_bones["foot_l"].head, edit_bones["foot_l"].tail, edit_bones["foot_l"].roll))),
        ("ik_foot_pole_l", "tasty_root", LazyInit(lambda: (edit_bones["calf_l"].head + Vector((0, -0.75, 0)) * scale,
                                                           edit_bones["calf_l"].head + Vector((0, -0.75, -0.2)) * scale,
                                                           0))),

        ("ik_hand_parent_r", "tasty_root",
         LazyInit(lambda: (edit_bones["hand_r"].head, edit_bones["hand_r"].tail, edit_bones["hand_r"].roll))),
        ("ik_hand_target_r", "ik_hand_parent_r",
         LazyInit(lambda: (edit_bones["hand_r"].head, edit_bones["hand_r"].tail, edit_bones["hand_r"].roll))),
        ("ik_finger_thumb_r", "ik_hand_parent_r", LazyInit(lambda: (
            edit_bones["thumb_03_r"].tail, 2 * edit_bones["thumb_03_r"].tail - edit_bones["thumb_03_r"].head,
            edit_bones["thumb_03_r"].roll))),
        ("ik_finger_index_r", "ik_hand_parent_r", LazyInit(lambda: (
            edit_bones["index_03_r"].tail, 2 * edit_bones["index_03_r"].tail - edit_bones["index_03_r"].head,
            edit_bones["index_03_r"].roll))),
        ("ik_finger_middle_r", "ik_hand_parent_r", LazyInit(lambda: (
            edit_bones["middle_03_r"].tail, 2 * edit_bones["middle_03_r"].tail - edit_bones["middle_03_r"].head,
            edit_bones["middle_03_r"].roll))),
        ("ik_finger_ring_r", "ik_hand_parent_r", LazyInit(lambda: (
            edit_bones["ring_03_r"].tail, 2 * edit_bones["ring_03_r"].tail - edit_bones["ring_03_r"].head,
            edit_bones["ring_03_r"].roll))),
        ("ik_finger_pinky_r", "ik_hand_parent_r", LazyInit(lambda: (
            edit_bones["pinky_03_r"].tail, 2 * edit_bones["pinky_03_r"].tail - edit_bones["pinky_03_r"].head,
            edit_bones["pinky_03_r"].roll))),
        ("ik_hand_pole_r", "tasty_root", LazyInit(lambda: (edit_bones["lowerarm_r"].head + Vector((0, 0.75, 0)) * scale,
                                                           edit_bones["lowerarm_r"].head + Vector(
                                                               (0, 0.75, -0.2)) * scale, 0))),

        ("ik_hand_parent_l", "tasty_root",
         LazyInit(lambda: (edit_bones["hand_l"].head, edit_bones["hand_l"].tail, edit_bones["hand_l"].roll))),
        ("ik_hand_target_l", "ik_hand_parent_l",
         LazyInit(lambda: (edit_bones["hand_l"].head, edit_bones["hand_l"].tail, edit_bones["hand_l"].roll))),
        ("ik_finger_thumb_l", "ik_hand_parent_l", LazyInit(lambda: (
            edit_bones["thumb_03_l"].tail, 2 * edit_bones["thumb_03_l"].tail - edit_bones["thumb_03_l"].head,
            edit_bones["thumb_03_l"].roll))),
        ("ik_finger_index_l", "ik_hand_parent_l", LazyInit(lambda: (
            edit_bones["index_03_l"].tail, 2 * edit_bones["index_03_l"].tail - edit_bones["index_03_l"].head,
            edit_bones["index_03_l"].roll))),
        ("ik_finger_middle_l", "ik_hand_parent_l", LazyInit(lambda: (
            edit_bones["middle_03_l"].tail, 2 * edit_bones["middle_03_l"].tail - edit_bones["middle_03_l"].head,
            edit_bones["middle_03_l"].roll))),
        ("ik_finger_ring_l", "ik_hand_parent_l", LazyInit(lambda: (
            edit_bones["ring_03_l"].tail, 2 * edit_bones["ring_03_l"].tail - edit_bones["ring_03_l"].head,
            edit_bones["ring_03_l"].roll))),
        ("ik_finger_pinky_l", "ik_hand_parent_l", LazyInit(lambda: (
            edit_bones["pinky_03_l"].tail, 2 * edit_bones["pinky_03_l"].tail - edit_bones["pinky_03_l"].head,
            edit_bones["pinky_03_l"].roll))),
        ("ik_hand_pole_l", "tasty_root", LazyInit(lambda: (edit_bones["lowerarm_l"].head + Vector((0, 0.75, 0)) * scale,
                                                           edit_bones["lowerarm_l"].head + Vector(
                                                               (0, 0.75, -0.2)) * scale, 0))),

        ("index_control_r", "index_metacarpal_r", LazyInit(
            lambda: (edit_bones["index_01_r"].head, edit_bones["index_01_r"].tail, edit_bones["index_01_r"].roll))),
        ("middle_control_r", "middle_metacarpal_r", LazyInit(
            lambda: (edit_bones["middle_01_r"].head, edit_bones["middle_01_r"].tail, edit_bones["middle_01_r"].roll))),
        ("ring_control_r", "ring_metacarpal_r",
         LazyInit(lambda: (edit_bones["ring_01_r"].head, edit_bones["ring_01_r"].tail, edit_bones["ring_01_r"].roll))),
        ("pinky_control_r", "pinky_metacarpal_r", LazyInit(
            lambda: (edit_bones["pinky_01_r"].head, edit_bones["pinky_01_r"].tail, edit_bones["pinky_01_r"].roll))),

        ("index_control_l", "index_metacarpal_l", LazyInit(
            lambda: (edit_bones["index_01_l"].head, edit_bones["index_01_l"].tail, edit_bones["index_01_l"].roll))),
        ("middle_control_l", "middle_metacarpal_l", LazyInit(
            lambda: (edit_bones["middle_01_l"].head, edit_bones["middle_01_l"].tail, edit_bones["middle_01_l"].roll))),
        ("ring_control_l", "ring_metacarpal_l",
         LazyInit(lambda: (edit_bones["ring_01_l"].head, edit_bones["ring_01_l"].tail, edit_bones["ring_01_l"].roll))),
        ("pinky_control_l", "pinky_metacarpal_l", LazyInit(
            lambda: (edit_bones["pinky_01_l"].head, edit_bones["pinky_01_l"].tail, edit_bones["pinky_01_l"].roll))),

        ("ik_dog_ball_r", "ik_foot_target_r", LazyInit(lambda: (
            edit_bones["dog_ball_r"].tail, 2 * edit_bones["dog_ball_r"].tail - edit_bones["dog_ball_r"].head,
            edit_bones["dog_ball_r"].roll))),
        ("ik_dog_ball_l", "ik_foot_target_l", LazyInit(lambda: (
            edit_bones["dog_ball_l"].tail, 2 * edit_bones["dog_ball_l"].tail - edit_bones["dog_ball_l"].head,
            edit_bones["dog_ball_l"].roll))),

        ("ik_wolf_ball_r", "ik_foot_target_r", LazyInit(lambda: (
            edit_bones["wolf_ball_r"].tail, 2 * edit_bones["wolf_ball_r"].tail - edit_bones["wolf_ball_r"].head,
            edit_bones["wolf_ball_r"].roll))),
        ("ik_wolf_ball_l", "ik_foot_target_l", LazyInit(lambda: (
            edit_bones["wolf_ball_l"].tail, 2 * edit_bones["wolf_ball_l"].tail - edit_bones["wolf_ball_l"].head,
            edit_bones["wolf_ball_l"].roll))),

        ("eye_control_parent", "tasty_root", LazyInit(lambda: (edit_bones["head"].head + Vector((0, -0.675, 0)) * scale,
                                                               edit_bones["head"].head + Vector((0, -0.7, 0)) * scale,
                                                               0))),
        ("eye_control_r", "eye_control_parent", LazyInit(lambda: (
            edit_bones["eye_control_parent"].head - Vector((0.0325, 0, 0)) * scale,
            edit_bones["eye_control_parent"].tail - Vector((0.0325, 0, 0)) * scale, 0))),
        ("eye_control_l", "eye_control_parent", LazyInit(lambda: (
            edit_bones["eye_control_parent"].head + Vector((0.0325, 0, 0)) * scale,
            edit_bones["eye_control_parent"].tail + Vector((0.0325, 0, 0)) * scale, 0)))
    ]
    ...
    for bone in pose_bones:
        ...
        if any(bone.bone.parent_recursive, lambda parent: parent.name in face_root_bones):
            ...
            if not any(["eyelid", "eye_lid"],
                       lambda filter: filter in bone.name.casefold()) and bone.custom_shape is None:
