...
def apply_valorant_rig(master_skeleton: bpy.types.Armature):
    addon_dir = os.path.dirname(os.path.splitext(__file__)[0])
    with bpy.data.libraries.load(os.path.join(addon_dir, "ValorantRig.blend")) as (data_from, data_to):

        for obj in data_from.objects:
            if not bpy.data.objects.get(obj):
                data_to.objects.append(obj)

    hidden_group = master_skeleton.data.collections.new(name='HiddenGroup')
    ik_group = master_skeleton.data.collections.new(name='IKGroup')
    pole_group = master_skeleton.data.collections.new(name='PoleGroup')
    twist_group = master_skeleton.data.collections.new(name='TwistGroup')
    face_group = master_skeleton.data.collections.new(name='FaceGroup')
    extra_group = master_skeleton.data.collections.new(name='ExtraGroup')
    
    global shape_scale
    shape_scale = scale_fix # if bpy.context.active_object.dimensions.z < 4 else scale
    ...
    ik_rig_bones = [
        ...
        (hand_pole_r, edit_bones.get(elbow_r).head + Vector((-0.33, 0, 0)) * shape_scale,
         edit_bones.get(elbow_r).head + Vector((-0.33, 0, -0.05)) * shape_scale, 0),
        (hand_pole_l, edit_bones.get(elbow_l).head + Vector((-0.33, 0, 0)) * shape_scale,
         edit_bones.get(elbow_l).head + Vector((-0.33, 0, -0.05)) * shape_scale, 0),
        (foot_pole_r, edit_bones.get(knee_r).head + Vector((0.33, 0, 0)) * shape_scale,
         edit_bones.get(knee_r).head + Vector((0.33, 0, -0.05)) * shape_scale, 0),
        (foot_pole_l, edit_bones.get(knee_l).head + Vector((0.33, 0, 0)) * shape_scale,
         edit_bones.get(knee_l).head + Vector((0.33, 0, -0.05)) * shape_scale, 0),

        (eye_mid_control, edit_bones.get(head).head + Vector((0.675, 0, 0)) * shape_scale,
         edit_bones.get(head).head + Vector((0.7, 0, 0)) * shape_scale, 0)
    ]
    ...
    dependent_rig_bones = [
        # IK bone chain hands
        (shoulder_r_ik, edit_bones.get(shoulder_r).head, edit_bones.get(shoulder_r).tail,
         edit_bones.get(shoulder_r).roll, clavicle_r),
        (shoulder_l_ik, edit_bones.get(shoulder_l).head, edit_bones.get(shoulder_l).tail,
         edit_bones.get(shoulder_l).roll, clavicle_l),
        (elbow_r_ik, edit_bones.get(elbow_r).head, edit_bones.get(elbow_r).tail, edit_bones.get(elbow_r).roll,
         shoulder_r_ik),
        (elbow_l_ik, edit_bones.get(elbow_l).head, edit_bones.get(elbow_l).tail, edit_bones.get(elbow_l).roll,
         shoulder_l_ik),
        ...
        (eye_r_control, edit_bones.get(eye_mid_control).head + Vector((0, 0.0325, 0)) * shape_scale,
         edit_bones.get(eye_mid_control).tail + Vector((0, 0.0325, 0)) * shape_scale, 0, eye_mid_control),
        (eye_l_control, edit_bones.get(eye_mid_control).head + Vector((0, -0.0325, 0)) * shape_scale,
         edit_bones.get(eye_mid_control).tail + Vector((0, -0.0325, 0)) * shape_scale, 0, eye_mid_control)
    ]
    ...
    for pose_bone in pose_bones:
        ...
        pose_bone.custom_shape_scale_xyz = 0.03 * shape_scale, 0.03 * shape_scale, 0.03 * shape_scale
    ...
    custom_shape_bones = [
        (root, 'RIG_Root', shape_scale, (90, 0, 0)),
        (hips, 'RIG_Torso', 0.75 * shape_scale),
        # pelvis,
        (spine_01, 'RIG_Hips', shape_scale),
        (spine_02, 'RIG_Hips', shape_scale),
        (spine_03, 'RIG_Hips', shape_scale),
        (spine_04, 'RIG_Hips', shape_scale),
        (neck_01, 'RIG_Hips', shape_scale, (90, 0, 0)),
        (neck_02, 'RIG_Hips', shape_scale),
        (head, 'RIG_Hips', shape_scale, (90, 0, 0)),
        (clavicle_r, 'RIG_Shoulder', 1),
        (clavicle_l, 'RIG_Shoulder', 1),

        (toe_r, 'RIG_Toe', 4 * shape_scale, (90, 0, 180)),
        (toe_l, 'RIG_Toe', 4 * shape_scale, (-90, 0, 180)),

        (hand_r_ik, 'RIG_Hand', 0.35 * shape_scale, (0, 0, 90)),
        (hand_l_ik, 'RIG_Hand', 0.35 * shape_scale),
        (foot_r_ik, 'RIG_FootR', shape_scale, (0, 90, 0)),
        (foot_l_ik, 'RIG_FootL', shape_scale, (0, 180, 0)),

        (hand_pole_r, 'RIG_Tweak', 1.5 * shape_scale),
        (hand_pole_l, 'RIG_Tweak', 1.5 * shape_scale),
        (foot_pole_r, 'RIG_Tweak', 1.5 * shape_scale),
        (foot_pole_l, 'RIG_Tweak', 1.5 * shape_scale),

        (eye_mid_control, 'RIG_EyeTrackMid', 30 * shape_scale),
        (eye_r_control, 'RIG_EyeTrackInd', 30 * shape_scale),
        (eye_l_control, 'RIG_EyeTrackInd', 30 * shape_scale)]
    ...
    limb_bones = ['Shoulder', 'Elbow', 'Hip', 'Knee', 'Foot']
    for limb_bone in limb_bones:
        for side in sides:
            ...
            bone.custom_shape_rotation_euler = radians(0), radians(0), radians(0)
    ...
    for hand_bone in hand_bones:
        for side in sides:
            ...
            while i <= 3:
                ...
                custom_shape_bone.custom_shape_scale_xyz = shape_scale, shape_scale, shape_scale
                custom_shape_bone.custom_shape_rotation_euler = radians(0), radians(0), radians(0)
    ...
    for child in children:
        ...
        child.custom_shape_rotation_euler = radians(0), radians(0), radians(0)
        child.custom_shape_scale_xyz = shape_scale, shape_scale, shape_scale

    # other tweaks by enumeration of pose bones

    for pose_bone in pose_bones:
        if pose_bone.children == 0:
    ...
    # target, ik, pole
    ik_bones = [
        (elbow_r_ik, hand_r_ik, hand_pole_r, '85'),
        (elbow_l_ik, hand_l_ik, hand_pole_l, '-85'),
        (knee_r_ik, foot_r_ik, foot_pole_r, '-180'),
        (knee_l_ik, foot_l_ik, foot_pole_l, '-180'),
    ]
    ...
    # Hide the hidden bones collection
    hidden_group.is_visible = False
...
def constraint_object(child: bpy.types.Object, parent: bpy.types.Object, bone: str,
...
class RigProperties(bpy.types.PropertyGroup):
    ik_fk: bpy.props.FloatProperty(name="IK FK Switch", description="Influence of IK vs FK", default=1.0, min=0.0,
                                   max=1.0, subtype="PERCENTAGE", update=ik_changed);
